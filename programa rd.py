...
# Função para upload de arquivos para o Cloudflare R2
def upload_file_to_r2(file, filename):
    try:
        r2_client.upload_fileobj(file, R2_BUCKET_NAME, filename)
        print(f"Arquivo {filename} enviado para o R2 com sucesso!")
        return True
    except Exception as e:
        print(f"Erro ao enviar o arquivo para o R2: {e}")
        return False

# Funções de banco de dados e controle de permissões
def init_db():
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute('''
